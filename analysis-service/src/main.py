...
try:
    manager_channel_id = os.environ.get("MANAGER_CHANNEL_ID", "C0A4F762FE0")
...
def _trigger_coach_alert(case_id: str, analysis_result: Any) -> None:
    """
    Trigger Agent 5 Coach Alert if generated by Orchestrator.
    
    Sends immediate Slack alert to the sales rep.
    """
    if not coach_agent or not slack_client or not db:
        return
    
    try:
        # Check if Orchestrator Phase 6 generated an alert
        alert = getattr(analysis_result, 'coach_alert', None)
        
        if not alert:
            # No alert generated or Orchestrator didn't run Phase 6
            return
        ...
        response = slack_client.chat_postMessage(
            channel=alert.rep_id,
            blocks=blocks,
            text=f"{alert.alert_type}: {alert.store_name}"
        )
        
        if response.get("ok"):
            # Update the stored alert with the sent TS
            slack_ts = response.get("ts", "")
            coach_agent.save_alert(
                alert=alert,
                slack_channel=alert.rep_id,
                slack_ts=slack_ts,
            )
            logger.info(f"Coach alert notification sent for case {case_id}: {alert.alert_type} (ts: {slack_ts})")
        else:
    ...
    # --- Phase 5: Manager Alert ---
    try:
        manager_alert = getattr(analysis_result, 'manager_alert', None)
        if manager_alert:
            manager_channel = os.environ.get("MANAGER_CHANNEL_ID", "C0A4F762FE0")
            
            blocks = coach_agent.build_manager_alert_blocks(manager_alert)
            
            resp = slack_client.chat_postMessage(
                channel=manager_channel,
                blocks=blocks,
                text=f"🚨 業務異常警示: {manager_alert.rep_name}"
            )
            
            if resp.get("ok"):
                slack_ts = resp.get("ts", "")
                coach_agent.save_manager_alert(
                    alert=manager_alert, 
                    slack_channel=manager_channel, 
                    slack_ts=slack_ts
                )
                logger.info(f"Manager alert sent for case {case_id} to {manager_channel} (ts: {slack_ts})")
            else:
                logger.error(f"Failed to send manager alert: {resp}")
                
    except Exception as e:
        logger.error(f"Error sending manager alert for {case_id}: {e}", exc_info=True)
...
@flask_app.route("/coach/chat", methods=["POST"])
def coach_chat_response():
    """
    Handle chat interaction with the Coach Agent (Agent 5).
    Called by Slack App when user replies in a coach thread.
    
    Payload:
    {
        "threadId": "123456.789",  # Slack thread_ts
        "channelId": "U12345",     # Slack channel/user ID
        "message": "User question"
    }
    """
    if not coach_agent:
        return jsonify({"status": "error", "message": "CoachAgent not initialized"}), 503
    ...
    if not thread_ts or not user_message:
        return jsonify({"status": "error", "message": "Missing required fields"}), 400
        
    try:
        # 1. Look up alert context by thread_ts to find Case ID
        alert_context = coach_agent.get_alert_by_thread_ts(thread_ts)
        
        if not alert_context:
             return jsonify({
                 "status": "ignored", 
                 "message": "Thread not associated with any active alert"
             }), 200

        case_id = alert_context.get("caseId")
        original_suggestion = alert_context.get("suggestion", "")
        
        # Fetch thread history to provide context
        thread_history = []
        if slack_client:
            try:
                # Use slack_client to get replies
                hist_resp = slack_client.conversations_replies(
                    channel=channel_id, 
                    ts=thread_ts,
                    limit=10  # Context window
                )
                if hist_resp.get("ok"):
                    for msg in hist_resp.get("messages", []):
                        # Improve robustness: Check if it's the bot itself
                        is_bot = "bot_id" in msg or "app_id" in msg
        ...
        # 2. Call respond with context
        response_text = coach_agent.respond(
            case_id=case_id,
