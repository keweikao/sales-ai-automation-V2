...
- **Given** a sales rep has recorded a sales call on iPhone (m4a format, Chinese language) with 2-3 participants
...
- **Given** an audio file from another unit stored in GCS bucket
...
- **Given** multiple audio files uploaded simultaneously (10+ files)
...
- **Given** a transcription with speaker diarization completed (3 speakers identified)
...
- **Given** participant profiles are generated
...
- **Given** sentiment analysis is completed
   **When** displayed in Slack notification
   **Then** system should:
...
- **Given** product needs are extracted
   **When** displayed in Slack notification
   **Then** system should:
...
- **Given** competitor analysis is completed
   **When** displayed in Slack notification
   **Then** system should:
...
- **Given** a transcription is completed with discussion about specific features (e.g., 掃碼點餐)
...
- **Given** questionnaire analysis is completed
   **When** displayed in Slack notification
   **Then** system should:
...
- **Given** multiple features discussed in single conversation
...
- **Given** questionnaire templates are configurable
...
- **Given** all specialized analyses are completed (participant, sentiment, needs, competitor, questionnaire)
...
- **Given** sales coaching is completed
...
- **Given** all AI analyses are completed for case "202501-IC001"
...
- **Given** sales rep clicks [💬 追問 AI] button
...
- **Given** sales rep asks "這個客戶適合哪個價格方案?" in the analysis thread
...
- **Given** sales rep clicks [⭐ 給回饋] button
   **When** button is clicked
   **Then** system should:
...
- **Given** sales rep submits feedback via Slack modal
...
- **Given** transcription completes with quality score <60
...
- **Given** transcription fails due to download error
...
- **Given** system processes 100 files in a day
...
- **Given** a file is fully processed (transcription + analysis)
...
- **Given** user accesses admin dashboard
   **When** page loads
...
- **Phase 0**: 執行 6 個 POC 驗證（輸出：`research.md`）
- **Phase 1**: 詳細設計（輸出：`data-model.md`, `contracts/`, `quickstart.md`）
- **Phase 2**: 建立實作任務（輸出：`tasks.md`）
- **Phase 3+**: 開始 Sprint 1-7 實作
