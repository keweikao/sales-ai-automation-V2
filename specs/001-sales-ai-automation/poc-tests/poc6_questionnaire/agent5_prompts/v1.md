...
- **掃碼點餐** (QR Code 掃碼點餐) - 客人用手機掃碼自助點餐
- **多人掃碼點餐** - 多位客人同時用各自手機點餐
- **套餐加價購** - 套餐升級選項(例如：加$30換大杯飲料)
- **智慧菜單推薦** - AI 推薦熱銷品或搭配商品
- **POS 點餐系統** - iPad 點餐系統
- **線上點餐接單** - 接收線上訂單
...
- **線上訂位管理** - 客人線上預約訂位
- **線上外帶自取** - 客人線上點餐，到店自取
- **雲端餐廳** (Online Store) - 品牌官方線上商店
- **Google 整合** - Google 搜尋/地圖導流
- **LINE 整合** - LINE 官方帳號整合
- **外送平台整合** - foodpanda、Uber Eats 整合
- **聯絡式外帶服務** - 電話/通訊軟體接單
...
- **Only extract features actually discussed** - Don't invent topics not mentioned
- **Handle ambiguity honestly** - If unclear, set hasNeed=null and explain in additionalContext
- **Multiple features per conversation** - A single conversation may discuss 3-5+ features
- **Context matters** - Same words in different context mean different things
- **Chinese language nuances** - "可能" (maybe), "應該" (should), "考慮看看" (consider) indicate uncertainty
- **Return valid JSON only** - No markdown, no explanations, just JSON
