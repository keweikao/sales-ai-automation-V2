...
Uses Google AI Gemini API for audio transcription.
Migrated from: src/transcription/gemini_pipeline.py
"""
...
class AudioChunker:
    """Simple audio chunker for long files."""

    def __init__(self, target_chunk_duration: int = 600, overlap_duration: int = 2):
        self.target_chunk_duration = target_chunk_duration
        self.overlap_duration = overlap_duration

    def create_chunks(self, total_duration: float) -> List[Dict[str, Any]]:
        """Create chunk definitions for audio file."""
        ...
        while current_start < total_duration:
            duration = min(self.target_chunk_duration, total_duration - current_start)
            chunks.append({
                "chunk_id": chunk_id,
                "start": current_start,
                "duration": duration
            })
            current_start += self.target_chunk_duration - self.overlap_duration
            chunk_id += 1
                        ...
                        for attempt in range(max_retries):
                            try:
                                response = self.model.generate_content(
                                    [audio_file, prompt],
                                    generation_config={"temperature": 0.1, "max_output_tokens": 8192, "response_mime_type": "text/plain"}
                                )
                                break
